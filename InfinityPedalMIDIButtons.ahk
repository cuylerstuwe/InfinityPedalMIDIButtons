...
#include MIDICCKeypress.ahk

MIDI_DEVICE_INDEX := 2 ; CHANGE THIS TO THE INDEX OF YOUR VIRTUAL MIDI CABLE

LEFT_MIDI_CC := 50
MIDDLE_MIDI_CC := 51
RIGHT_MIDI_CC := 52

UseMIDIDevice(MIDI_DEVICE_INDEX)

; IMPLEMENTED MIDI Event Pump w/ Saved State for the Infinity 3-Pedal HID Input Device
...
onLeftDown() {
    global LEFT_MIDI_CC
    SendMIDIKeypress(LEFT_MIDI_CC)
}

onMiddleDown() {
    global MIDDLE_MIDI_CC
    SendMIDIKeypress(MIDDLE_MIDI_CC)
}

onRightDown() {
    global RIGHT_MIDI_CC
    SendMIDIKeypress(RIGHT_MIDI_CC)
}
